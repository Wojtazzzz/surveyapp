...
@section('content')
    <div class="w-full flex flex-col justify-center gap-5">
        <h2 class="text-3xl text-center font-bold">Edit Survey</h2>

        <div class="container mx-auto">
            @include('inc.validation-errors')

            <form method="POST" action="{{ route('surveys.update', ['survey' => $survey]) }}">
                @csrf
                @method('PATCH')
                <div class="mb-6">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Name</label>
                    <input type="text" id="name" name="name" class="input @error('name') input-invalid @enderror"
                        placeholder="Example Survey" value="{{ old('name', $survey->name) }}" required>
                </div>
